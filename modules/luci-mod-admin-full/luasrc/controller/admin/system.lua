...
function index()
	...
	entry({"admin", "system"}, alias("admin", "system", "system"), _("System"), 70).index = true
	...
	elseif luci.http.formvalue("image") or luci.http.formvalue("step") then
		...
		elseif step == 2 then
			...
			if keep == "" then 
				...
				fork_exec("killall dropbear uhttpd; /ugw/script/reset_data.sh; sleep 1; /sbin/sysupgrade %s %q" %{ keep, image_tmp })
			end
		end
